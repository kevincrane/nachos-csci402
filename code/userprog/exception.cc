...
void Acquire_Syscall(unsigned int strPtr, int length) {
  ...
  sprintf(msg, "s%dn%s", ACQUIRE, name);

  outPacketHeader.to = rand() % NUM_SERVERS;
  outMailHeader.to = 0;
  outMailHeader.from = currentThread->getThreadNum();
  outPacketHeader.from = postOffice->getPostAddress();
  outMailHeader.length = strlen(msg) + 1;

  printf("Sending a message to %i.\n", outPacketHeader.to);
  bool success = postOffice->Send(outPacketHeader, outMailHeader, msg);
  ...
  postOffice->Receive(currentThread->getThreadNum(), &inPacketHeader, &inMailHeader, response);
  DEBUG('r', "%i: Acquire: Response, %s\n", currentThread->getProcessID(),response);
  printf("ERROR.\n");
  if(response[0] == 'e') {
    if((response[1]-48) == BADINDEX1 && (response[2]-48) == BADINDEX2) {
...
void Release_Syscall(unsigned int strPtr, int length) {
  ...
  sprintf(msg, "s%di%s", RELEASE, name);
...
void Wait_Syscall(unsigned int cvStrPtr, int cvLength, unsigned int lockStrPtr, int lockLength) {
  ...
  copyin(cvStrPtr, MAX_SIZE, cvName);
  copyin(lockStrPtr, MAX_SIZE, lockName);

  if(cvLength >= MAX_SIZE - 4) {
    printf("ERROR: This CV name is too long.\n");
    return;
  }
  
  if(lockLength >= MAX_SIZE - 4) {
    printf("ERROR: This lock name is too long.\n");
    return;
  }

  sprintf(msg, "s%dc%sl%s", WAIT, cvName, lockName);
...
void Signal_Syscall(unsigned int cvStrPtr, int cvLength, unsigned int lockStrPtr, int lockLength) {
  PacketHeader inPacketHeader;
  PacketHeader outPacketHeader;
  MailHeader   inMailHeader;
  MailHeader   outMailHeader;
  ...
  sprintf(msg, "s%dc%sl%s", SIGNAL, cvName, lockName);
  DEBUG('r', "msg: %s\n", msg);
  ...
  if(response[0] == 'e') {
    ...
    if((response[1]-48) == BADINDEX1 && (response[2]-48) == BADINDEX2) {
      ...
      DEBUG('r',"NOTE: Empty waiting list.\n");
    }
		else if((response[1]-48)=='1' && (response[2]-48)=='8') {
      DEBUG('r',"NOTE: This is wrong lock.\n");
    }
...
void Broadcast_Syscall(unsigned int cvStrPtr, int cvLength, unsigned int lockStrPtr, int lockLength) {
  PacketHeader inPacketHeader;
  PacketHeader outPacketHeader;
  MailHeader   inMailHeader;
  MailHeader   outMailHeader;
  ...
  sprintf(msg, "s%dc%sl%s", BROADCAST, cvName, lockName);
  DEBUG('r', "msg: %s\n", msg);
...
void DestroyLock_Syscall(unsigned int strPtr, int length) {
  ...
  sprintf(msg, "s%di%s", DESTROYLOCK, name);
...
void DestroyCondition_Syscall(unsigned int strPtr, int length) {
  ...
  sprintf(msg, "s%dn%s", DESTROYCV, name);
  outPacketHeader.to = rand() % NUM_SERVERS;
  outMailHeader.to = 0;
  outMailHeader.from = currentThread->getThreadNum();
  outPacketHeader.from = postOffice->getPostAddress();
...
void DestroyMV_Syscall(unsigned int strPtr, int length) {
  ...
  sprintf(message, "s%dn%s", DESTROYMV, name);
...
void SetMV_Syscall(unsigned int strPtr, int length, int value, int arrayIndex) {
  ...
  sprintf(message, "s%dn%si%da%d", SET, name, value, arrayIndex);
...
int GetMV_Syscall(int strPtr, int length, int arrayIndex) {
  ...
  sprintf(message, "s%di%si%d", GET, name, arrayIndex);
